...
st.header('heart desease prediction 2')
@st.cache_data
def load_data():
...
numeric_features=['age','sex','trestbps','thalach','oldpeak','slope','ca']
...
st.pyplot()
st.markdown('---')
#st.write(pd.get_dummies(data['thal'],prefix='TH'))
...
st.write('standart scaler',X)
X,y=preprocess_inputs(data,RobustScaler())
st.write('robust scaler', X)
X,y=preprocess_inputs(data,MinMaxScaler())
st.write('min max scaler',X)
#--------------------
X_train,X_test,y_train,y_test=train_test_split(X,y,train_size=0.8,random_state=0)
for model in [LogisticRegression(),SVC(),MLPClassifier()]:
    model.fit(X_train,y_train)
    st.write(f'{model} accuracy:',model.score(X_test,y_test))

#st.write(pd.get_dummies(X['thal']))
